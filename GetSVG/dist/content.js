...
`;function m(e,t){let n=t.getBoundingClientRect();e.style.left=`${n.left}px`,e.style.top=`${n.top}px`,e.style.width=`${n.width}px`,e.style.height=`${n.height}px`}function h(e){let t=e.getBoundingClientRect();return{left:t.left,top:t.top>60?t.top-50:t.bottom+4}}function g(e){let t=document.getElementById(`svgr-toast`);if(!t){t=document.createElement(`div`),t.id=`svgr-toast`,t.style.cssText=`position:fixed;top:24px;left:50%;transform:translateX(-50%);z-index:2147483647;pointer-events:none`,document.documentElement.appendChild(t);let e=t.attachShadow({mode:`open`}),n=document.createElement(`style`);n.textContent=`
  ...
  `,r.appendChild(i);let a=document.createElement(`div`);a.className=`p`,r.appendChild(a),setTimeout(()=>n.remove(),3e3)}function v(e){e.querySelectorAll(`.popup`).forEach(e=>e.remove())}function y(e,t,n,r){v(e);let i=document.createElement(`div`);i.className=`popup`;let a=document.createElement(`button`);a.className=`btn`,a.textContent=`Copy`,a.addEventListener(`click`,async t=>{t.stopPropagation();try{let t=await d(r);if(!t){v(e),g(`Copy failed!`);return}await f(t.svg),v(e),g(`SVG Copied!`)}catch{v(e),g(`Copy failed!`)}});let o=document.createElement(`button`);o.className=`btn ghost`,o.textContent=`Save`,o.addEventListener(`click`,t=>{t.stopPropagation(),d(r).then(t=>{if(!t){v(e),g(`Save failed!`);return}chrome.runtime.sendMessage({action:`save-svg`,svg:t.svg,filename:t.filename}),v(e),g(`SVG Saved!`)}).catch(()=>{v(e),g(`Save failed!`)})});let s=document.createElement(`button`);s.className=`btn ghost`,s.textContent=`Data URI`,s.addEventListener(`click`,async t=>{t.stopPropagation();let n=await chrome.storage.sync.get([`proKey`,`proVerified`]);if(!(n.proKey&&n.proVerified)){v(e),g(`Get Pro to enable Copy as Data URI`),chrome.runtime.sendMessage({action:`open-pro-popup`});return}try{let t=await d(r);if(!t){v(e),g(`Copy failed!`);return}await f(`data:image/svg+xml;base64,${btoa(unescape(encodeURIComponent(t.svg)))}`),v(e),g(`Data URI Copied!`)}catch{v(e),g(`Copy failed!`)}}),i.appendChild(a),i.appendChild(o),i.appendChild(s),e.appendChild(i);let{left:c,top:l}=h(t);i.style.left=`${c}px`,i.style.top=`${l}px`}async function b(){r&&x(),g(`Highlighting SVGs...`),await new Promise(e=>requestAnimationFrame(()=>requestAnimationFrame(e)));let e=u();if(e.length===0)return;let t=document.createElement(`div`);t.id=`svgr-root`,t.style.cssText=`position:fixed;inset:0;pointer-events:none;z-index:2147483647;overflow:visible`,document.documentElement.appendChild(t);let a=t.attachShadow({mode:`open`}),o=document.createElement(`style`);o.textContent=p,a.appendChild(o);let s=document.createElement(`div`);s.className=`tooltip`,s.style.display=`none`,a.appendChild(s);for(let t of e){let e=n.get(t.id);if(!e)continue;let r=document.createElement(`div`);r.className=`overlay`,r.dataset.id=t.id,m(r,e),a.appendChild(r),r.addEventListener(`mouseenter`,()=>{s.innerHTML=`<span class="badge">${t.method}</span>${t.width}×${t.height}px`;let e=r.getBoundingClientRect();s.style.left=`${e.left}px`,s.style.top=`${e.top>36?e.top-34:e.bottom+4}px`,s.style.display=`block`}),r.addEventListener(`mouseleave`,()=>{s.style.display=`none`}),r.addEventListener(`click`,n=>{n.stopPropagation(),y(a,r,t,e)})}let c=()=>{a.querySelectorAll(`.overlay`).forEach(e=>{let t=n.get(e.dataset.id);t&&m(e,t)})};window.addEventListener(`scroll`,c,{capture:!0,passive:!0}),window.addEventListener(`resize`,c,{passive:!0});let l=e=>{e.composedPath().includes(t)||v(a)};document.addEventListener(`click`,l,{capture:!0}),i.push(()=>window.removeEventListener(`scroll`,c,{capture:!0}),()=>window.removeEventListener(`resize`,c),()=>document.removeEventListener(`click`,l,{capture:!0})),r=t}function x(){i.splice(0).forEach(e=>e()),r?.remove(),r=null}function S(e){let t=e.closest(`svg`)??e;t.scrollIntoView({behavior:`smooth`,block:`center`,inline:`center`});let n=null,r=0,i=performance.now()+2500,a=()=>{let o=t.getBoundingClientRect().top;if(o===n){if(r++,r>=4){_(e);return}}else r=0;n=o,performance.now()<i?requestAnimationFrame(a):_(e)};requestAnimationFrame(a)}return chrome.runtime.onMessage.addListener((e,i,a)=>{if(e.action===`show-pro-toast`)return g(`Get Pro to enable Copy as Data URI`),a({success:!0}),!0;if(e.action===`copy-svg`||e.action===`download-svg`){let n=t;return n?(d(n).then(t=>{if(!t){a({success:!1,error:`No SVG found at right-clicked element`});return}e.action===`copy-svg`?f(t.svg).then(()=>{g(`SVG Copied!`),_(n),a({success:!0})}).catch(e=>a({success:!1,error:String(e)})):a({success:!0,svg:t.svg,filename:t.filename})}),!0):(a({success:!1,error:`No element right-clicked`}),!0)}if(e.action===`copy-svg-uri`){let e=t;return e?(d(e).then(t=>{if(!t){a({success:!1,error:`No SVG found at right-clicked element`});return}f(`data:image/svg+xml;base64,${btoa(unescape(encodeURIComponent(t.svg)))}`).then(()=>{g(`Data URI Copied!`),_(e),a({success:!0})}).catch(e=>a({success:!1,error:String(e)}))}),!0):(a({success:!1,error:`No element right-clicked`}),!0)}if(e.action===`detect-svgs`){try{a({success:!0,items:u()})}catch(e){a({success:!1,error:String(e)})}return!0}if(e.action===`get-svg-by-id`){let t=n.get(e.id);return t?(d(t).then(e=>{a(e?{success:!0,svg:e.svg,filename:e.filename}:{success:!1,error:`Could not extract SVG`})}).catch(e=>a({success:!1,error:String(e)})),!0):(a({success:!1,error:`Element not found`}),!0)}if(e.action===`get-highlight-state`)return a({active:!!r}),!0;if(e.action===`toggle-highlight`)return r?(x(),a({success:!0,active:!1})):b().then(()=>{a({success:!0,active:!!r})}),!0;if(e.action===`locate-svg`){let t=n.get(e.id);return t?(S(t),a({success:!0}),!0):(a({success:!1,error:`Element not found`}),!0)}return!0}),e.detectAllSVGs=u,e})({});